function shareStreamUsingAntMediaServer(stream) {
    ...
    wrapper.callbacks('onopen', function() {
        ...
        wrapper.publish(stream, uid, function(isPublished, error) {
            if (isPublished === true) {
                var resultingURL = 'https://www.hiringmirror.com/screen/ant/?s=' + uid;
                var hlsURL = 'https://www.hiringmirror.com/screen/ant/?s=' + uid + '&hls=true';

                var html = "<title>Unique Room URL</title><link rel='icon' href='images/desktopCapture128.png'><h1 style='text-align:center; margin-top:20px;'>Copy following private URL:</h1>";
                ...
                html += "<p><label style='font-weight:bold'>HLS Live Stream URL:</label><br><input type='text' value='" + hlsURL + "' style='text-align:left;width:100%;font-size:1.2em; margin-bottom:20px;'></p>";
